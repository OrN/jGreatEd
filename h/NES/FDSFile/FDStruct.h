...
#define		NES_FILE_SIGNATURE			0x1A53454E		//  $4E $45 $53 $1A
...
typedef union _tagNES_HEADER_FLAG6
{
	BYTE			DATA;
	struct
	{
		BYTE		LO_MAPPER : 4;
		BYTE		NAMETABLE_ALT : 1;
		BYTE		HAS_TRAINER : 1;
		BYTE		HAS_PRG_RAM : 1;
		BYTE		NAMETABLE_ARRANGEMENT : 1;
	};
} NES_HEADER_FLAG6, * PNES_HEADER_FLAG6;

typedef union _tagNES_HEADER_FLAG7
{
	BYTE			DATA;
	struct
	{
...
} NES_HEADER_FLAG7, * PNES_HEADER_FLAG7;
...
typedef struct _tagNES_HEADER
{
	...
	BYTE				PRG_ROM;
	BYTE				CHR_ROM;
	NES_HEADER_FLAG6	FLAGS_6;
	NES_HEADER_FLAG7	FLAGS_7;
	BYTE				MAPPER;
	BYTE				PRG_CHR_ROM_MSB;
	BYTE				PRG_RAM_SIZE;
	BYTE				CHR_RAM_SIZE;
	BYTE				CPU_TIMING;
	BYTE				VS_SYSTEM_TYPE;
	BYTE				MISC_ROM_COUNT;
	BYTE				DEFAULT_EXPANSION_DEVICE;
